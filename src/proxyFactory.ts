...
import {
    ProxyFactory,
    ProxyFactory__factory as ProxyFactoryBase,
...
export class Factory {
    ...
    async multiCallDeployProxyAndUpgradeProxy(
        ...
        const factory = await this.getFactory();
        const multiCallArgs = await this.encodeMultiCallDeployProxyAndUpgradeProxyArgs(
            logicAddress,
            initCallData,
            salt
        );
    ...
    async encodeMultiCallDeployUpgradeableArgs(
        ...
        const [deployCreate, upgradeProxy] = await this.encodeMultiCallUpgradeProxyArgs(
            implementationBase,
            initCallData,
            constructorArgs,
            salt
    ...
    async encodeMultiCallUpgradeProxyArgs(
        ...
        const implementationContractAddress = await this.calculateDeployCreateAddress(
            factory.address
        );
        const upgradeProxyCallData = factory.interface.encodeFunctionData(
            "upgradeProxy",
    ...
    async calculateDeployCreateAddress(
  ...
  */
    ...
    async deployCreateAndRegister(
        ...
        const factory = await this.getFactory();
        const logicContract: any = await this.ethers.getContractFactory(contractName);
