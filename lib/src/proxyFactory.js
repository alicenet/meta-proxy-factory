...
class Factory {
    ...
    async deploy(overrides) {
        let factoryBase = await hardhat_1.ethers.getContractFactoryFromArtifact(proxyFactoryArtifact);
    ...
    async init() {
        this.factory = await hardhat_1.ethers.getContractAtFromArtifact(proxyFactoryArtifact, this.factory.address);
        this._initialized = true;
    }
    async changeFactory(factoryAddress) {
        this.factory = await hardhat_1.ethers.getContractAtFromArtifact(proxyFactoryArtifact, factoryAddress);
        this._initialized = true;
    }
