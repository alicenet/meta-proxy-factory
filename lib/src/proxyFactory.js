...
class Factory {
    ...
    constructor(ethers, factoryAddress) {
        ...
        if (factoryAddress !== undefined) {
            this.factory = new ethers.Contract(factoryAddress, ProxyFactory_json_1.default.abi, ethers.provider);
        }
        else {
            this.factory = new ethers.Contract(ethers.constants.AddressZero, ProxyFactory_json_1.default.abi, ethers.provider);
        }
    }
    async getFactory() {
    ...
    async deploy(overrides) {
        let factoryBase = await hardhat_1.ethers.getContractFactoryFromArtifact(ProxyFactory_json_1.default);
    ...
    async init() {
        this.factory = await hardhat_1.ethers.getContractAtFromArtifact(ProxyFactory_json_1.default, this.factory.address);
        this._initialized = true;
    }
    async changeFactory(factoryAddress) {
        this.factory = await hardhat_1.ethers.getContractAtFromArtifact(ProxyFactory_json_1.default, factoryAddress);
        this._initialized = true;
    }
